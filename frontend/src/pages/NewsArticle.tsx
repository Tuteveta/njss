  ...
  if (!article) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <Link to="/news" className="inline-flex items-center gap-2 text-sm font-medium text-[hsl(210,70%,25%)] hover:underline">
  ...
  return (
    <div>
      ...
      <div className="relative h-72 md:h-96 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[hsl(210,70%,15%)]/70" />
        <div className="absolute inset-0 flex flex-col justify-end max-w-7xl mx-auto px-6 lg:px-10 pb-10 w-full left-0 right-0">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-xs text-blue-200 mb-4">
            <Link to="/" className="hover:text-white">Home</Link>
            <ChevronRight className="w-3 h-3" />
            <Link to="/news" className="hover:text-white">News & Updates</Link>
            <ChevronRight className="w-3 h-3" />
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <article className="flex-1 min-w-0">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 md:p-10">
                ...
                <div className="space-y-6">
                  {article.body.map((block, i) => (
                    <div key={i}>
                      {block.heading && (
                        <h2 className="text-lg font-bold text-[hsl(210,70%,25%)] mb-2">{block.heading}</h2>
                      )}
                ...
                <div className="mt-10 pt-6 border-t border-gray-100 flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant="secondary">Workers Compensation</Badge>
              ...
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {prev ? (
                  <Link
                    to={`/news/${prev.slug}`}
                    className="flex items-start gap-3 bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md hover:-translate-y-0.5 transition-all group"
                  >
                    <ArrowLeft className="w-4 h-4 text-gray-400 mt-1 shrink-0 group-hover:text-[hsl(210,70%,25%)]" />
                    <div className="min-w-0">
                      <p className="text-xs text-gray-400 mb-0.5">Previous</p>
                      <p className="text-sm font-medium text-gray-800 group-hover:text-[hsl(210,70%,25%)] leading-snug line-clamp-2">{prev.title}</p>
                    </div>
                  </Link>
                ) : <div />}
                {next && (
                  <Link
                    to={`/news/${next.slug}`}
                    className="flex items-start gap-3 bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md hover:-translate-y-0.5 transition-all group text-right justify-end"
                  >
                    <div className="min-w-0">
                      <p className="text-xs text-gray-400 mb-0.5">Next</p>
                      <p className="text-sm font-medium text-gray-800 group-hover:text-[hsl(210,70%,25%)] leading-snug line-clamp-2">{next.title}</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-gray-400 mt-1 shrink-0 group-hover:text-[hsl(210,70%,25%)]" />
                  </Link>
                )}
              </div>
            </article>
            ...
            <aside className="lg:w-72 shrink-0 space-y-6">
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm font-medium text-[hsl(210,70%,25%)] hover:underline"
              >
                  ...
                  <div className="space-y-4">
                    {related.map((r) => (
                      <Link key={r.id} to={`/news/${r.slug}`} className="flex gap-3 group">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-gray-800 group-hover:text-[hsl(210,70%,25%)] leading-snug line-clamp-2">{r.title}</p>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="space-y-3">
                  {allNews.filter((n) => n.slug !== slug).slice(0, 4).map((r) => (
                    <Link key={r.id} to={`/news/${r.slug}`} className="flex items-start gap-2 group">
                      <ChevronRight className="w-3.5 h-3.5 text-gray-300 mt-0.5 shrink-0 group-hover:text-[hsl(210,70%,25%)]" />
                      <div>
                        <p className="text-xs font-medium text-gray-700 group-hover:text-[hsl(210,70%,25%)] leading-snug line-clamp-2">{r.title}</p>
              ...
              {/* CTA */}
              <div className="bg-[hsl(210,70%,25%)] rounded-xl p-5 text-white">
                ...
                >
                  Contact OWC <ArrowRight className="w-3 h-3" />
                </Link>
              </div>
            </aside>
